      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.recipientList}>
            {recipients.map((item) => (
              <View key={item.id} style={styles.recipientItem}>
                <View style={[styles.recipientIcon, { backgroundColor: item.color }]}>
        ...
        <TouchableOpacity style={styles.promoCard}>
          <View style={styles.promoContent}>
            <Icon name="bolt" size={16} color="#da291c" style={{ marginRight: 10 }} />
            <Text style={styles.promoText}>Set up recurring payments for utilities and bills.</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  actionLabel: { fontSize: 12, fontWeight: '500', textAlign: 'center', marginTop: 8, color: '#333' },
  recipientsSection: { marginVertical: 10 },
  recipientList: { paddingLeft: 15 },
  recipientItem: { width: 75, alignItems: 'center', marginRight: 10 },
  recipientIcon: { width: 50, height: 50, borderRadius: 25, justifyContent: 'center', alignItems: 'center', marginBottom: 5 },
