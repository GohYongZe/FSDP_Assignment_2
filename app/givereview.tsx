...
export default function GiveReviewScreen() {
  ...
  const recipientName = params.recipientName as string; // payer
  const recipientAccountNo = params.recipientAccountNo as string; // payer account
  const amountStr = params.amount as string;
  const senderAccountNo = params.senderAccountNo as string; // requester (me)
  const senderName = params.senderName as string; // requester name
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const { error } = await supabase
            ...
            .insert({
                sender_account_no: senderAccountNo, // me (requester)
                receiver_account_no: recipientAccountNo, // them (payer target)
                amount: amount,
                status: 'pending',
                description: `Request from ${senderName}`
            });

      if (error) throw error;

      router.push({
          pathname: '/givesuccess',
          params: {
              amount: amount.toFixed(2),
              recipientName, // Whom I requested from
              recipientAccountNo,
        ...
        Alert.alert("Request Failed", error.message || "An unknown error occurred.");
    } finally {
        setLoading(false);
    }
  };
    ...
    <SafeAreaView className="flex-1 bg-white">
      <View className="flex-1">
        ...
        <View className="flex-row items-center p-4">
            ...
            <Text className="text-xl font-bold ml-4">Review Request</Text>
        </View>
        
        <View className="items-center py-8">
            <Text className="text-gray-500 text-lg">Amount to request</Text>
        ...
        <View className="p-6">
            ...
            >
                {loading ? <ActivityIndicator color="white" /> : <Text className="text-white font-bold text-lg">Confirm Request</Text>}
            </TouchableOpacity>
        </View>
