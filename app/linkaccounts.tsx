...
export default function LinkAccounts() {
  ...
  const fetchAndStoreCurrentUser = async (): Promise<UserAccount | null> => {
    try {
  ...
  const fetchData = async (user: UserAccount) => {
    ...
    if (incomingError) {
      ...
      await Promise.all((incoming || []).map(async (req) => {
        const name = await fetchNameByAccountNo(req.accountNo);
        
        if (req.requestStatus === 'Pending') {
           pendingIncoming.push({ ...req, requesterName: name || undefined });
  ...
  const fetchNameByAccountNo = async (accountNo: string): Promise<string | null> => {
    try {
      // localaccounts
  ...
  const handleRequestAction = async (requesterAccountNo: string, newStatus: string) => {
  ...
  const handleAcknowledge = async (account: LinkedAccount) => {
    ...
    if (error) {
        ...
        Alert.alert("System", "Unable to delete notification. Database permission denied.");
    } else {
        // Success
        fetchData(currentUser);
    }
  ...
  return (
    <ScrollView className="flex-1 bg-gray-50 p-4">
      {/* Header (for now)*/}
      <Text className="text-2xl font-bold mb-6 text-gray-800">Linked Accounts</Text>
      ...
      {outgoingRequests.length > 0 && (
        <View className="mb-6">
          <Text className="text-lg font-semibold mb-3 text-gray-700">Sent Requests</Text>
          {outgoingRequests.map((request, index) => {
            const key = request.created_at ? request.created_at + index : `out-${request.accountNo}-${request.linkedWith}`;
            return (
              <View key={key} className="bg-white p-4 rounded-lg mb-3 shadow-sm border border-gray-100 flex-row justify-between items-center">
                <View>
                  <Text className="text-base font-medium text-gray-800">{request.nickName}</Text>
                  <Text className="text-sm text-gray-500">{request.linkedWith}</Text>
                  <Text className="text-xs text-yellow-600 mt-1 font-semibold">Pending Acceptance</Text>
                </View>
                <TouchableOpacity 
      ...
      {/* Linked Accounts */}
      <View className="mb-8">
        <Text className="text-lg font-semibold mb-3 text-gray-700">My Linked Accounts</Text>
        {linkedAccounts.length === 0 ? (
          <Text className="text-gray-500 italic p-2 bg-white rounded-lg">No linked accounts yet.</Text>
        ) : (
          linkedAccounts.map((account, index) => {
             ...
             const key = account.created_at ? account.created_at + index : `${account.accountNo}-${account.linkedWith}`;

             if (isRejected) {
                return (
                    <View key={key} className="bg-red-50 p-4 rounded-lg mb-3 shadow-sm border border-red-100 flex-row justify-between items-center">
                        <View className="flex-1 mr-2">
                           <Text className="text-base font-bold text-red-800">Request Rejected</Text>
                           <Text className="text-sm text-red-600">
                               Request to {account.nickName} ({account.linkedWith}) was rejected.
                           </Text>
                        </View>
                        <TouchableOpacity 
                        ...
                        >
                            <Text className="text-red-800 font-semibold text-xs">Acknowledge</Text>
                        </TouchableOpacity>
                    </View>
                );
             }
             ...
             return (
              ...
              >
                <View>
                  <Text className="text-base font-medium text-gray-800">{account.nickName}</Text>
                  <Text className="text-sm text-gray-500">{account.linkedWith}</Text>
                </View>
                <View className="flex-row items-center gap-2">
                  <View className={`px-2 py-1 rounded ${getStatusColor(account.requestStatus)}`}>
                    <Text className={`text-xs font-bold ${getStatusColor(account.requestStatus).split(' ')[0]}`}>
                      {account.requestStatus}
                    </Text>
                  </View>
                  {isAccepted && (
                    <TouchableOpacity onPress={() => handleUnlink(account)} className="p-1">
      ...
      {/* Incoming Requests */}
      <View className="mb-8">
        <Text className="text-lg font-semibold mb-3 text-gray-700">Incoming Requests</Text>
        {incomingRequests.length === 0 ? (
          <Text className="text-gray-500 italic p-2 bg-white rounded-lg">No new requests.</Text>
        ) : (
            incomingRequests.map((request, index) => {
              const key = request.created_at ? request.created_at + index : `req-${request.accountNo}-${request.linkedWith}`;
              return (
                <View key={key} className="bg-white p-4 rounded-lg mb-3 shadow-sm border border-gray-100">
                  <View className="mb-3">
                    <Text className="text-base text-gray-800">
                      Request from: <Text className="font-bold">{request.requesterName || request.accountNo}</Text>
                    </Text>
                    <Text className="text-sm text-gray-500">{request.accountNo}</Text>
                  </View>
                  
                  <View className="flex-row gap-2">
                    <TouchableOpacity
                      onPress={() => handleRequestAction(request.accountNo, 'Accepted')}
                      className="bg-green-600 px-4 py-2 rounded-md flex-1 items-center"
                    >
                      <Text className="text-white font-semibold">Accept</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                      onPress={() => handleRequestAction(request.accountNo, 'Rejected')}
